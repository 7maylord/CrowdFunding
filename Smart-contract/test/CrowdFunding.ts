...
describe("CrowdFunding Contract", () => {
  const deployCrowdFundingContract = async () => {
    ...
    const Token = await ethers.getContractFactory("CTK");
    const token = await Token.deploy();
    await token.waitForDeployment();

    // Transfer tokens to contributors
    await token.transfer(donor1.address, 100); 
    await token.transfer(donor2.address, 100);
  ...
  describe("Creating a Fundraising Campaign", () => {
    it("Should allow owner to create a fundraising campaign", async () => {
      ...
      await expect(crowdFunding.createFundRaising("Health Issue", "Help save Labake", owner, goal, deadline))
        .to.emit(crowdFunding, "FundRaisingCreated")
        .withArgs(0, goal, deadline, owner.address);

      const campaign = await crowdFunding.fundRaisings(0);
    ...
    it("Should fail if goal is zero", async () => {
      ...
      await expect(crowdFunding.createFundRaising("Health Issue", "Help save Labake", owner, goal, deadline)).to.be.revertedWith("Goal must be greater than zero");
    });
  });
  ...
  describe("Donating to Fundraising", () => {
    it("Should allow users to donate", async () => {
      ...
      await expect(crowdFunding.connect(donor1).donate(0, ethers.parseEther("5")))
  ...
  describe("Ending a Fundraising Campaign", () => {
    it("Should allow creator to end campaign after deadline", async () => {
      ...
      await expect(crowdFunding.endFundRaising(1))
        .to.emit(crowdFunding, "FundRaisingEnded")
        .withArgs(0, false);
    ...
    it("Should fail if campaign is still active", async () => {
      ...
      await expect(crowdFunding.endFundRaising(1)).to.be.revertedWith("Fundraising is still active");
    });
  });
});
